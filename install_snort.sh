...
#install DAQ 2.0.6
wget https://www.snort.org/downloads/snort/daq-2.0.6.tar.gz -i /root/snort_src
tar -xvzf /root/snort_src/daq-2.0.6.tar.gz && cd daq-2.0.6 && ./configure && make -j3 && sudo make install

#cd /root/snort_src

#install snort 2.9.9.0
wget https://www.snort.org/downloads/snort/snort-2.9.9.0.tar.gz -i /root/snort_src
tar -xvzf /root/snort_src/snort-2.9.9.0.tar.gz && cd snort-2.9.9.0 && ./configure --enable-sourcefire && make -j3 && sudo make install
...
cp /root/snort_src/snort-2.9.9.0/etc/*.conf* /etc/snort
cp /root/snort_src/snort-2.9.9.0/etc/*.map /etc/snort
cp /root/snort_src/snort-2.9.9.0/etc/*.dtd /etc/snort

#cd /root/snort_src/snort-2.9.9.0/src/dynamic-preprocessors/build/usr/local/lib/snort_dynamicpreprocessor/
cp /root/snort_src/snort-2.9.9.0/src/dynamic-preprocessors/build/usr/local/lib/snort_dynamicpreprocessor/* /usr/local/lib/snort_dynamicpreprocessor/
